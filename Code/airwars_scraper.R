## Airwars Airstrike Civilian Casualty Scraper
## Steffi Colao | Michael Everett
## Scraper Functional as of August 2022
...
##### the series of numbers below (e.g. 2:32) refers to the series of urls for a search
##### after the first 'home' url seen above. In this case, the second page after the home page has a /2/ pasted into the url and the last page
##### has a /32/ pasted into the url (and every number in between pasted into other observations for the pages in between).
##### Since this series of pages is different from the last series of urls (without declared strike), the sequence was changed from 1:45 to 2:32
core.links <- data.frame(Links = str_c('https://airwars.org/civilian-casualties/page/', 2:32, '/?country=somalia&belligerent=us-forces&strike_status=declared-strike')) #create the URLs for the 45 pages 
...
write_csv(out.info, str_c(Sys.Date(), "_civilian_casualty_summary.csv"))
